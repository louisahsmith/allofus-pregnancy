...
hipps_w_demo %>%
  ...
  mutate(conc = "High-quality episodes") %>%
...
hipps_cleaned <- hipps_w_demo %>%
  ...
  mutate(
    ...
    across(
      c(gender_cat, race_eth_cat, edu_cat, income_cat, marital_status_cat),
...
demo_table <- hipps_cleaned %>%
  ...
  mutate(across(where(is.factor), ~ fct_lump_min(.x, min = 20)),
    state_cat = fct_infreq(state_cat),
    state_cat = fct_lump_min(state_cat, min = 20),
    orientation_cat = fct_relevel(orientation_cat, "Straight", "Bisexual", "Gay/lesbian", "None"),
    orientation_cat = fct_recode(orientation_cat, NULL = "No answer")
  ) %>%
  tbl_summary(
