...
```{r}
all_people <- tbl(con, "cb_search_person")
tally(all_people) %>% collect()
male <- filter(all_people, sex_at_birth == "Male")
tally(male) %>% collect()
no_ehr <- filter(filter(all_people, sex_at_birth != "Male"), has_ehr_data == 0)
tally(no_ehr) %>% collect()
```
```{r}
